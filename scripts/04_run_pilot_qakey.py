...
def _load_model_config(model_config_path: str) -> dict[str, object]:
    ...
    if model_id not in PRIMARY_MODEL_REPO_IDS:
        raise PilotRunError(f"Unsupported model_id {model_id!r} for v1 QA-Key pilot.")
    return model_config
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run QA-Key pilot inference for one model/prompt.")
    ...
    try:
        ...
        print(f"QA-Key pilot inference failed: {error}", file=sys.stderr)
        return 1
