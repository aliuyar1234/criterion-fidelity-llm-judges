...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the canonical BioRubric fact table for M2.")
    parser.add_argument("--config", required=True, help="Path to the BioRubric source config.")
    ...
    try:
        ...
        print(f"Canonical fact table build failed: {error}", file=sys.stderr)
        return 1
    return 0
