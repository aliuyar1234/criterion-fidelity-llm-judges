...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run M6 full standard-prompt baselines.")
    parser.add_argument(
        "--config", required=True, help="Path to the full-standard experiment config."
    )
    ...
    try:
        ...
        if not slice_specs:
            raise FullStandardRunError("No full-standard slices matched the requested filters.")
        ...
        print(f"M6 full standard baseline run failed: {error}", file=sys.stderr)
        return 1
